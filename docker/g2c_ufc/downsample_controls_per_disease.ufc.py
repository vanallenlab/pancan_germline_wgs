...
def main():
    ...
    with open(args.log_file, "a") as f:
        f.write(f"{sample_size2}\t{(sample_size1 - sample_size2)}\t{((sample_size1 - sample_size2)/sample_size1).round(2) * 100}\tRemove samples with unknown cancer status.\n")
    ...
    with open(args.log_file, "a") as f:
        f.write(f"{sample_size3}\t{(sample_size2 - sample_size3)}\t{((sample_size2 - sample_size3)/sample_size2).round(2) * 100}\tRemove samples that are not controls nor {args.cancer_subtype.replace('|',',')} diagnosis.\n") 
    ...
    with open(args.log_file, "a") as f:
        f.write(f"{sample_size4}\t{(sample_size3 - sample_size4)}\t{((sample_size3 - sample_size4)/sample_size3).round(2) * 100}\tRemove samples that are not {args.sex_karyotypes}.\n")
    ...
    with open(args.log_file, "a") as f:
        f.write(f"{sample_size5}\t{(sample_size4 - sample_size5)}\t{((sample_size4 - sample_size5)/sample_size4).round(2) * 100}\tExcluded {len(familial_set)} due to relatedness with other individuals.\n")
