...
# Define constants
primary_contigs = ['chr{}'.format(x+1) for x in range(22)] + ['chrX', 'chrY']
...
def startup_report(inputs):
    ...
    start_log = start_log.format(date_str)
...
def report_status(all_status, title=None):
    ...
    if title is None:
        dat_str = datetime.now().strftime("%Y-%m-%d at %H:%M:%S")
        print('Status as of {}:'.format(date_str))
    else:
        print(title)
    for status, contigs in sorted(status_groups.items(), 
                                  key=lambda x: len(x[1]), reverse=True):
        print('  [{:,}] {}: {}'.format(len(contigs), status, ', '.join(sorted(list(contigs)))))
    print('')


def check_if_staged(output_json_uri):
    """
    Returns true if an outputs .json file is found in the expected location

    Note that we bookkeep chromosome-sharded workflow outputs using a 
    reserved .json file, which should *only* be present if the workflow was 
    successful and all of the outputs were able to be staged in --staging-bucket.
    """

    return g2cpy.check_gcp_uris([output_json_uri])


def relocate_outputs(workflow_id, staging_bucket, action='cp', timeout=30, 
                     max_retries=20, verbose=False):
    """
...
def main():
    ...
    if args.name is None:
        method_name = path.splitext(path.basename(args.wdl))[0]
    else:
    ...
    input_json_fmt = sub('[/]+$', '', args.base_directory) + \
                     '/cromshell/inputs/' + method_name + '.{}.inputs.json'
    prev_wids_fmt = sub('[/]+$', '', args.base_directory) + \
                    '/cromshell/job_ids/' + method_name + '.{}.job_ids.list'
    ...
    # Check if any status is not 'staged'
    while len(set(all_status.keys()).difference({'staged'})) > 0:
        print('Not all contigs yet staged. Entering another cycle of ' + \
        ...
        for contig in contigs:
            ...
            while True:
                ...
                if status == 'staged':
                    break
                if check_if_staged(output_json_uri):
                    status = 'staged'
                    break
                ...
                if status == 'succeeded':
                    relocate_outputs(wid, output_bucket)
                    status = 'staged'
                    g2cpy.collect_workflow_trash(wids, bucket, args.wdl, args.dumpster)
