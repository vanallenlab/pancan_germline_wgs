...
bench.barplot <- function(plot.dat, set.colors, title=NULL, strata.labels=NULL, strata.label.srt=NULL, reverse.strata=FALSE, strata.title=NULL, y.title="Variants", bar.buffer=0.2, parmar=c(2, 2.5, 2, 2.5)){
  ...
  right.y.lab.pos <- yaxis.legend(af.breaks, x=max(bar.x1), y.positions=right.y.mids,
               sep.wex=0.05*diff(par("usr")[1:2]), upper.limit=0.5,
               colors=hex2grey(set.pals[[n.sets]]), label.cex=4.5/6,
               lwd=2, return.label.pos=T)
    ...
    sapply(1:n.sets, function(k){
      ...
      if(sx %in% rownames(set.df)){
        ...
        rect(xleft=xs[k], xright=xs[k+1], ybottom=ys[-length(ys)], ytop=ys[-1],
             col=set.pals[[k]], border=set.pals[[k]], lwd=0.25)
      }
    })
...
plot.all.benchmarking <- function(bench.dat, out.prefix, set.colors, strata.labels=NULL, reverse.strata=FALSE, strata.title=NULL, ref.title="ref. data", y.title="Variants", title.prefix="Concordance of", title.preposition="with", common.af=0.01){
  ...
  for(vc in vcs){
    ...
    pdf(paste(out.prefix, vc, "barplot.pdf", sep="."), height=2.25, width=2.6)
  ...
  has.short.vars <- any(unlist(lapply(bench.dat, function(l){
    ...
    any(sub.df$class %in% c("snv", "indel") & clean.breaks(sub.df$af) < common.af)
  })))
  if(has.short.vars){
    short.vsc.dat <- lapply(bench.dat, extract.bench.res, vc.elig=c("snv", "indel"),
    ...
    pdf(paste(out.prefix, "short_subclasses.barplot.pdf", sep="."), height=2.25, width=2.6)
    bench.barplot(short.vsc.dat, set.colors, title=bench.title,
                  strata.labels=vsc.strata.labels, strata.title="Short variant class",
                  y.title=y.title)
    dev.off()
  }
  ...
  if(has.svs){
    ...
    pdf(paste(out.prefix, "sv_subclasses.barplot.pdf", sep="."), height=2.25, width=2.6)
...
parser$add_argument("--ppv-by-af", metavar=".tsv", action="append",
                    help=paste("PPV benchmarking results by AF. Can be",
                               "specified multiple times for multiple",
                               "evaluation interval sets."))
parser$add_argument("--set-names", metavar="string", action="append",
...
#              "set_names" = c("Easy", "Hard"),
#              "ref_title" = "gnomAD v4.1",
#              "common.af" = 0.001,
#              "out_prefix" = "~/scratch/g2c.qc.test")

# Load sensitivity & PPV data
sens.by.af <- load.benchmark.tsvs(args$sens_by_af, args$set_names)
ppv.by.af <- load.benchmark.tsvs(args$ppv_by_af, args$set_names)
...
if(!is.null(sens.by.af)){
  ...
  plot.all.benchmarking(sens.by.af, paste(args$out_prefix, "sens_by_af", sep="."),
...
if(!is.null(ppv.by.af)){
  ...
  plot.all.benchmarking(ppv.by.af, paste(args$out_prefix, "ppv_by_af", sep="."),
                        set.colors, strata.labels=af.strata.labels,
                        reverse.strata=TRUE, strata.title=strata.title,
                        args$ref_title, y.title="Confirmation rate",
                        "Confirmation of", "versus", common.af=args$common.af)
}
