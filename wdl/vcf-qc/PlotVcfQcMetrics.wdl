...
task PlotSampleMetrics {
  ...
  String pop_opt = if defined(ancestry_labels) then "--ancestry-labels ~{select_first([basename(ancestry_labels), ''])}" else ""
  String pheno_opt = if defined(phenotype_labels) then "--phenotype-labels ~{select_first([basename(phenotype_labels), ''])}"  else ""
